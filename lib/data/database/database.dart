...
class Database extends _$Database {
  ...
  Future<int> deleteFolder({@required int folderId}) async {
    var result = await (delete(folders)
...
// Notes
  ...
  Future<int> changeNoteFolderId({@required int noteId,
  ...
  Future<int> setNoteDeletedStatus(
      {int newFolderId, @required int noteId, bool isDeleted = true}) {
    // Make a note as deleted or not, this won't remove the note from db, it just set isDeleted = true
    // If *newFolderId* isn't empty, meaning that it will change the note's folderId to the new folder id, this will be used when deleting a folder
    // After that the note with the deleted status will use the new folder id.

    return (update(notes)
      ..where((e) => e.id.equals(noteId))).write(
        NotesCompanion(
